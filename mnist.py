...
class FullyConnectedLayer(Layer, Parameterized):
    ...
    def __init__(self, input_layer, num_units, 
        ...
        self.b = np.random.randn(self.num_units) * init_stddev
    ...
    def bprop(self, output_grad):
        ...
        if self.activation_fun == None:
            ...
            activation_grad = self.activation_fun.bprop(output_grad)
        self.dW = np.dot(np.transpose(self.last_input), activation_grad) / n
        self.db = np.sum(activation_grad, axis=0) / n
        # the gradient wrt. the input should be calculated here
        grad_input = np.dot(activation_grad, np.transpose(self.W))
        return grad_input
...
class NeuralNetwork:
    ...
    def backpropagate(self, Y, Y_pred, upto=0):
        ...
        # TODO ##########################################
        active_layer = len(self.layers) - 1
        while active_layer > upto:
            active_layer = active_layer - 1
            next_grad = layers[active_layer].bprop(next_grad)
        return next_grad
    ...
    def sgd_epoch(self, X, Y, learning_rate, batch_size):
        ...
        for b in range(n_batches):
            ...
            if ((b+1) * batch_size) > X.shape[0]:
                X_batch = X[b*batch_size:]
                Y_batch = Y[b*batch_size:]
            else:
    ...
    def train(self, X, Y, X_valid, Y_valid, learning_rate=0.1, max_epochs=100, batch_size=64,
        ...
        for e in range(max_epochs+1):
            ...
            print('         valid_loss {:.4f}, valid error {:.4f}'.format(valid_loss, valid_error))
            if valid_error < 0.02:
                break
...
t0 = time.time()
nn.train(X_train, y_train, X_valid, y_valid, learning_rate=.1, 
         max_epochs=500, batch_size=64, descent_type = "sgd", y_one_hot=True)
t1 = time.time()
